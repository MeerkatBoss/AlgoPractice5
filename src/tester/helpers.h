...
{                                                                              \
    ...
    time_delta = (uint64_t)((time_end - time_start) / (CLOCKS_PER_SEC / 1000));\
} while (0)

#define TEST_TREE_INSERT(tested_tree, tdata) do                             \
{                                                                           \
    for (size_t data_index = 0; data_index < tdata.data_size; ++data_index) \
        tested_tree.insert(tdata.data[data_index]);                         \
} while(0)

#define TEST_TREE_ERASE(tested_tree, tdata) do                              \
{                                                                           \
    for (size_t data_index = 0; data_index < tdata.data_size; data_index+=2)\
